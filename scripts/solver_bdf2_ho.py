...
def solve_coupled_heat_equation(
    ...
    gfu_last_on_new_mesh = GridFunction(X)
    gfu_last2_on_new_mesh = GridFunction(X)
    vel_last, lagr_last = gfu_last_on_new_mesh.components
    vel_last2, lagr_last2 = gfu_last2_on_new_mesh.components
    ...
    dx_neg = dCut(levelset=lsetp1, domain_type=NEG,
                  definedonelements=els_hasneg, deformation=deformation)
    dx_if = dx(definedonelements=els_if)
    ds = dCut(levelset=lsetp1, domain_type=IF, definedonelements=els_if,
              deformation=deformation)
    dw = dFacetPatch(definedonelements=facets_ext, deformation=deformation)
    ...
    for inner_it in range(max_inner_it):
        ...
        for _i, _j in product(range(2), repeat=2):
            gfu_last_on_new_mesh.components[_i].components[_j].Set(
                shifted_eval(gfu_last.components[_i].components[_j],
    ...
    for it in range(2, int(t_end / dt) + 1):
        ...
        for inner_it in range(max_inner_it):
            ...
            deformation = lsetmeshadap.CalcDeformation(levelset_func)
            for _i, _j in product(range(2), repeat=2):
                gfu_last2_on_new_mesh.components[_i].components[_j].Set(
                    shifted_eval(gfu_last2.components[_i].components[_j],
